@extends("App::app")
@section('title',$q.'的搜索结果 - '.get_options('web_name', 'U87.PUB') )
@section('description',get_options('web_name', 'U87.PUB').'是'.get_options('title', config('app_name', 'U87.PUB')).','.$q.'是其中一个搜索结果.' )
@section('keywords', $q.','.get_options('title', config('app_name', 'U87.PUB')).','.get_options('web_name', 'U87.PUB') )
@section('content')
...
<div class="card order-5">
	<div class="card-header">
		<ol class="breadcrumb align-items-center" aria-label="breadcrumbs">
		  ...
		  <li class="breadcrumb-item d-none d-sm-block" aria-current="page">搜索结果</li>
		</ol>
	</div>
    @if($page->count())
