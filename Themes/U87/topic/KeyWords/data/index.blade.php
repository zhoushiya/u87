<div class="card order-5">
	<div class="card-header">
		<ol class="breadcrumb" aria-label="breadcrumbs">
		  ...
		  <img src="{{$data->icon}}" class="avatar avatar-xs tag-icon avatar-rounded me-1" alt="{{$data->tag->name}}">{{$data->name}}</a></li>
    ...
    <article class="col-md-12 p-3 border-bottom">
    	<div class="d-flex align-items-center justify-content-between">
    		...
    		<span class="d-flex" data-bs-toggle="tooltip" data-bs-placement="top" title="权限值 : {{$data['user']['class']['permission-value']}}">{!! $data->user->class->icon !!}</span>
    		<span class="badge badge-outline ms-1" style="color:{{$data->user->class->color}}; ">{{$data->user->class->name}}</span>
    		</div>
    		<div class="d-flex align-items-center opacity-50" data-bs-toggle="tooltip" data-bs-placement="top" title="发表于:{{$data->created_at}}">
    		...
    		<div class="d-flex align-items-center">
    		@if($data->essence>0)
    		<div data-bs-toggle="tooltip" data-bs-placement="top" title="精华贴">
    		...
    		@endif
    		@if($data->topping>0)
    		<div class="ms-1" data-bs-toggle="tooltip" data-bs-placement="top" title="置顶贴">
    		...
    		<div class="col-md-12 markdown home-article mt-2">
    			<a href="/{{$data->id}}.html" class="text-reset">
    				<h2>{{$data->title}}</h2>
    			<div class="home-summary">{{ \Hyperf\Utils\Str::limit(core_default(deOptions($data->options)["summary"],"未捕获到本文摘要")300) }}
    	...
    	<div class="row align-items-center justify-content-between mt-2">
    		...
    		<a href="/{{$data->id}}.html" class="text-reset text-decoration-none opacity-50 me-1 me-sm-3" data-bs-toggle="tooltip" data-bs-placement="bottom" title="浏览次数">
    		...
    		<a href="/{{$data->id}}.html" class="text-reset text-decoration-none opacity-50 me-1 me-sm-3" data-bs-toggle="tooltip" data-bs-placement="bottom" title="评论数"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-message-circle" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M0 0h24v24H0z" stroke="none"/><path d="M3 20l1.3-3.9A9 8 0 1 1 7.7 19L3 20M12 12v.01M8 12v.01M16 12v.01"/></svg> @php echo \App\Plugins\Comment\src\Model\TopicComment::query()->where(['topic_id'=>$data->id,'status'=>'publish'])->count(); @endphp
    		</a>
    		
    		</div>
    		<div class="col-auto">
    			<a href="javascript:void(0);" core-click="like-topic" topic-id="{{$data->id}}" class="text-reset text-decoration-none opacity-50 cursor-pointer me-1 me-sm-3" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="点赞">
    				<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M19.5 13.572l-7.5 7.428l-7.5 -7.428m0 0a5 5 0 1 1 7.5 -6.566a5 5 0 1 1 7.5 6.572" /></svg>
    				<span core-show="topic-likes">{{$data->like}}</span>
    			</a>
    			<a href="javascript:void(0);" core-click="star-topic" topic-id="{{$data->id}}" class="text-reset text-decoration-none opacity-50 cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="收藏">
